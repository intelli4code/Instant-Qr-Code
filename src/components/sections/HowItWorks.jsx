      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16 px-4">
          <motion.div 
            ...
            className="flex-1"
          >
            <h2 className="text-3xl md:text-5xl font-extrabold text-white mb-4 text-glow">How It Works</h2>
            <p className="text-on-surface-variant max-w-sm font-medium">Generate professional-grade QR codes in three simple steps.</p>
          </motion.div>
          <div className="h-px bg-sky-400/10 flex-1 hidden md:block mb-6"></div>
        </div>
          ...
          {steps.map((step, idx) => (
            ...
            >
              ...
              <div className="flex flex-col items-start gap-6 p-4">
                <div className="w-16 h-16 bg-background rounded-2xl flex items-center justify-center border border-sky-400/10 group-hover:border-primary/50 group-hover:scale-110 transition-all duration-500 shadow-xl">
                  <span className="material-symbols-outlined text-primary text-3xl font-bold">{step.icon}</span>
                </div>
                <div>
                  <h3 className="text-2xl font-bold text-white mb-3 tracking-tight group-hover:text-primary transition-colors">{step.title}</h3>
