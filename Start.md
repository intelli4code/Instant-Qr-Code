...
3. **App Shell:** Build `App.jsx`, pulling in the `Navbar`, `HeroGenerator`, `HowItWorks`, `FAQ`, and `Footer`. Apply the deep navy styling to the top half.
...
5. **Implement Logic:** Wire up the state in `GeneratorWidget` so that typing in `ContentTabs` triggers the `useDebounce` hook, which then passes the data to `PreviewPanel` to render via `qr-code-styling`.
