...
You are an expert frontend developer building a blazing-fast, client-side React application using Vite. The app is a frictionless, premium-feeling QR code generator. 
...
/src
  /components
    /layout
      - MainLayout.jsx (handles the 2-column grid)
      - Header.jsx (category toggles)
      - AdsterraBanner.jsx (placeholder for ads)
    /forms
      - ContentForm.jsx (dynamic inputs based on category)
      - DesignForm.jsx (tabs for Frame, Logo, Colors)
    /preview
      - QRPreview.jsx (sticky right column, handles loading state and rendering)
  /hooks
...
The UI uses a modern, two-column layout. 
* **Background:** Deep navy/dark blue background for the main app body.
* **Left Column (Controls):** Contains white "Cards" (rounded-xl, shadow-md, p-6). The cards behave like an accordion:
    * **Card 1: Content.** Contains the input fields.
    * **Card 2: Design.** Contains tabs for "Frame", "Logo", and "Color & Shape".
...
1. **Global State:** Create a central state object in `App.jsx` to hold all configurations:
...
3. **QR Generation (`qr-code-styling`):**
   * Use a ref to attach the generated canvas/SVG to the DOM in `QRPreview.jsx`.
   * Ensure the download button extracts directly from this instance.

## 💰 Monetization
Create a component called `<AdsterraBanner />`. For now, make it a `div` with a gray background, a dashed border, and text saying "Adsterra Placeholder - 728x90". Place one at the very top of the layout (above the main container) and one at the bottom.
...
3. Build the core layout (Left Column / Right Column split using Tailwind grid/flex).
4. Implement the State Management in `App.jsx`.
5. Build the Left Column: Top category toggles, Content Input Card, and Design Options Card.
6. Build the Right Column: The conditional loading ring, the `qr-code-styling` renderer, and the Download button.
