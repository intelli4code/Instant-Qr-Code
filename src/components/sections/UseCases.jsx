    ...
    <section id="features" className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-extrabold text-white mb-4 text-glow">Limitless Use Cases</h2>
        ...
        >
          {cases.map((useCase, idx) => (
            <motion.div
              ...
              className="glass-card-elevated p-8 rounded-2xl border border-sky-400/5 group transition-all"
            >
              <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 ${useCase.color}`}>
                <span className="material-symbols-outlined text-3xl font-bold">{useCase.icon}</span>
              </div>
              <h3 className="text-xl font-bold text-white mb-3 tracking-tight">{useCase.title}</h3>
              <p className="text-on-surface-variant text-sm leading-relaxed">{useCase.desc}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
